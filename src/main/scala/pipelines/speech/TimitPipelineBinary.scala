...
object TimitPipelineBinary extends Logging {
  val appName = "Timit"
  ...
  def run(sc: SparkContext, conf: TimitConfig) {
    ...
    val labels = ClassLabelIndicatorsFromIntLabels(2).apply(
      timitFeaturesData.train.labels.map(l => if (l == 0) 0 else 1)
    ...
    val actual = timitFeaturesData.test.labels.map(l => if (l == 0) 0 else 1).cache().setName("actual")
