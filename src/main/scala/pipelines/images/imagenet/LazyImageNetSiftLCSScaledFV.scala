...
object LazyImageNetSiftLcsScaledFV extends Serializable with Logging {
  val appName = "LazyImageNetSiftLcsScaledFV"
  ...
  def getSiftFeatures(
      conf: LazyImageNetSiftLcsScaledFVConfig,
  ...
  def run(sc: SparkContext, conf: LazyImageNetSiftLcsScaledFVConfig) {
  ...
  case class LazyImageNetSiftLcsScaledFVConfig(
  ...
  def parse(args: Array[String]): LazyImageNetSiftLcsScaledFVConfig = {
    new OptionParser[LazyImageNetSiftLcsScaledFVConfig](appName) {
      head(appName, "0.1")
    ...
    }.parse(args, LazyImageNetSiftLcsScaledFVConfig()).get
  }
